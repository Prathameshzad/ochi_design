...
function Header() {
    ...
    return (
    <div className={` flex flex-col md:flex-row backdrop-blur-md fixed z-50 w-full py-4 font-"Neue Montreal" justify-between  transition-transform ${visible ? 'translate-y-0 transition duration-200' : '-translate-y-full transition duration-200'}`}>
           <div className=' px-8'>
            <div className=' flex logo justify-self-start'>
            ...
            <div className='md:hidden order-2 absolute top-5 right-10'><Cross toggled={isOpen} toggle={setOpen} /></div>
            <div className='links md:flex md:gap-10 hidden order-1 items-start mx-2'>
                {["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index) => (
                    <a key={index} className={`text-lg capitalize  ${index === 4 && "md:ml-32 mr-6"}`}>{item}</a>
                ))}
            </div>
            {isOpen && (
                <div className='mt-14 border-t-2'>

                    <div className='links flex md:gap-10 flex-col items-start'>
                        {["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index) => (
                            <a key={index} className={`text-2xl  leading-10 capitalize my-4 mx-8 font-light ${index === 4 && "md:ml-32"}`}>{item}</a>
                        ))}
                    </div>
                </div>
            )
